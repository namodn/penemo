...
sub get_next_agent {
	...
	return penemo::agent->new( 
					...
					plugin_checks	=> $self->_plugin_checks($ip),
...
sub index_html_write {
	...
	my $line = '';
	my $green_button = penemo::core->html_button('index', 'green'); 
	my $red_button = penemo::core->html_button('index', 'red'); 
			...
			foreach my $agent (@agentlist) { 
				...
				if ($agent->get_group() eq "$group") { 
					if ($agent->get_index_error_detected()) { 
						print HTML "$red_button  ";
					} 
					else { 
						print HTML "$green_button  ";
					} 
...
sub write_persistent_data {
	...
	open (DATA, ">$data_dir/$ip") or die "Can't open $data_dir/$ip: $!\n";
		print DATA "$ping_errlev\t$http_errlev\t$snmp_errlev\t$current_tier",
			   "\t$notifications_sent\n";
	close DATA;
}
...
sub get_check_status {
	my ($self) = @_;
	unless ( ($self->get_ping_status()) && ($self->get_http_status()) && ($self->get_total_snmp_status()) ) {
		return 0;
	}
	else { 
		return 1;
...
sub agent_html_write
{
	...
	my $green_button = penemo::core->html_button('agent', 'green'); 
	my $red_button = penemo::core->html_button('agent', 'red'); 
